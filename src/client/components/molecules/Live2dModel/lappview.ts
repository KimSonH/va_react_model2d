...
export class LAppView {
  ...
  public render(): void {
    ...
    live2DManager.setViewMatrix(this._viewMatrix as CubismMatrix44)
  ...
  public initializeSprite(): void {
    if (canvas) {
      ...
      const initBackGroundTexture = (textureInfo: TextureInfo): void => {
        ...
        this._back = new LAppSprite(x, y, fwidth, fheight, textureInfo.id as WebGLTexture)
      }
  ...
  /**
   * Handle change lip motion
   *
   * @param value value of lip motion
   */
  public onChangeLipMotion(value: number): void {
    const live2DManager: LAppLive2DManager = LAppLive2DManager.getInstance()

    live2DManager.onMouthAction(value)
  }

  /**
   * Handle change expression motion
   *
   * @param name name of motion
   */
  public onChangeExpression(name?: string): void {
    const live2DManager: LAppLive2DManager = LAppLive2DManager.getInstance()
    if (name) {
      live2DManager.setExpression(name)
    } else live2DManager.resetExpression()
  }

  /**
   * Handle set random motion of model
   *
   * @param isRandom
   */
  public setRandomMotion(isRandom: boolean): void {
    const live2DManager: LAppLive2DManager = LAppLive2DManager.getInstance()

    live2DManager.resetMotion(isRandom)
  }
