...
export let s_instance: LAppWavFileHandler | undefined | null
...
export class LAppWavFileHandler {
  ...
  public loadWavFile(filePath: string): boolean {
    ...
    const asyncWavFileManager = (async () => {
      ...
      try {
        ...
        // チャンネル数
        this._wavFileInfo._numberOfChannels = this._byteReader.get16LittleEndian() as number
        // サンプリングレート
        ...
        // 量子化ビット数
        this._wavFileInfo._bitsPerSample = this._byteReader.get16LittleEndian() as number
        // fmtチャンクの拡張部分の読み飛ばし
        if (fmtChunkSize > 16) {
  ...
  public getPcmSample(): number {
    ...
    switch (this._wavFileInfo._bitsPerSample) {
      case 8:
        pcm32 = (this._byteReader.get8() as number) - 128
        pcm32 <<= 24
        break
      case 16:
        pcm32 = (this._byteReader.get16LittleEndian() as number) << 16
        break
      case 24:
        pcm32 = (this._byteReader.get24LittleEndian() as number) << 8
        break
      default:
        // 対応していないビット幅
        pcm32 = 0
  ...
  public releasePcmData(): void {
    for (let channelCount = 0; channelCount < this._wavFileInfo._numberOfChannels; channelCount++) {
      if (this._pcmData) delete this._pcmData[channelCount]
    }
...
export class ByteReader {
   ...
   */
  public get16LittleEndian(): number | undefined {
    if (this._fileDataView) {
      const ret =
        (this._fileDataView?.getUint8(this._readOffset + 1) << 8) | this._fileDataView?.getUint8(this._readOffset)
      this._readOffset += 2
      return ret
    }
    return 0
  ...
  public getCheckSignature(reference: string): boolean {
    ...
    for (let signatureOffset = 0; signatureOffset < 4; signatureOffset++) {
      getSignature[signatureOffset] = this.get8() as number
    }
    return (
  ...
  _fileByte: ArrayBuffer | null ///< ロードしたファイルのバイト列
