...
export class LAppSprite {
  ...
  public render(programId: WebGLProgram): void {
    ...
    if (this._firstDraw) {
      // 何番目のattribute変数か取得
      this._positionLocation = gl?.getAttribLocation(programId, 'position') as number
      gl?.enableVertexAttribArray(this._positionLocation)

      this._uvLocation = gl?.getAttribLocation(programId, 'uv') as number
      gl?.enableVertexAttribArray(this._uvLocation)

      // 何番目のuniform変数か取得
      this._textureLocation = gl?.getUniformLocation(programId, 'texture') as WebGLUniformLocation
      ...
      // uvバッファ、座標初期化
      {
        ...
        // uvバッファを作成
        this._uvBuffer = gl?.createBuffer() as WebGLBuffer
      }

      // 頂点バッファ、座標初期化
      {
        const maxWidth = canvas?.width || 0
        const maxHeight = canvas?.height || 0

        // 頂点データ
        this._positionArray = new Float32Array([
          ((this._rect?.right || 0) - maxWidth * 0.5) / (maxWidth * 0.5),
          ((this._rect?.up || 0) - maxHeight * 0.5) / (maxHeight * 0.5),
          ((this._rect?.left || 0) - maxWidth * 0.5) / (maxWidth * 0.5),
          (this._rect?.up || 0 - maxHeight * 0.5) / (maxHeight * 0.5),
          ((this._rect?.left || 0) - maxWidth * 0.5) / (maxWidth * 0.5),
          ((this._rect?.down || 0) - maxHeight * 0.5) / (maxHeight * 0.5),
          ((this._rect?.right || 0) - maxWidth * 0.5) / (maxWidth * 0.5),
          ((this._rect?.down || 0) - maxHeight * 0.5) / (maxHeight * 0.5)
        ])
