...
import type { CubismId, CubismIdHandle } from '@framework/id/cubismid'
...
export class LAppModel extends CubismUserModel {
  ...
  private setupModel(setting: ICubismModelSetting): void {
    ...
    const loadCubismPhysics = (): void => {
      if (this._modelSetting && this._modelSetting.getPhysicsFileName() != '') {
    ...
    const loadCubismPose = (): void => {
      if (this._modelSetting && this._modelSetting.getPoseFileName() != '') {
    ...
    const loadUserData = (): void => {
      if (this._modelSetting && this._modelSetting.getUserDataFile() != '') {
    ...
    const setupEyeBlinkIds = (): void => {
      const eyeBlinkIdCount: number = this._modelSetting?.getEyeBlinkParameterCount() as number

      for (let i = 0; i < eyeBlinkIdCount; ++i) {
        this._eyeBlinkIds.pushBack(this._modelSetting?.getEyeBlinkParameterId(i) as CubismId)
      }
    ...
    const setupLipSyncIds = (): void => {
      const lipSyncIdCount = this._modelSetting?.getLipSyncParameterCount() as number

      for (let i = 0; i < lipSyncIdCount; ++i) {
        this._lipSyncIds.pushBack(this._modelSetting?.getLipSyncParameterId(i) as CubismId)
      }
    ...
    const loadCubismMotion = (): void => {
      ...
      const motionGroupCount: number = this._modelSetting?.getMotionGroupCount() as number

      // モーションの総数を求める
      for (let i = 0; i < motionGroupCount; i++) {
        group[i] = this._modelSetting?.getMotionGroupName(i) as string
        this._allMotionCount += this._modelSetting?.getMotionCount(group[i]) as number
      }
      ...
      if (motionGroupCount == 0) {
        ...
        this.createRenderer()
        this.setupTextures()
        this.getRenderer().startUp(gl as WebGLRenderingContext)
      }
    }
  }
  ...
  private setupTextures(): void {
    ...
    if (this._state == LoadStep.LoadTexture) {
      // テクスチャ読み込み用
      const textureCount: number = this._modelSetting?.getTextureCount() as number
      ...
      for (let modelTextureNumber = 0; modelTextureNumber < textureCount; modelTextureNumber++) {
        ...
        texturePath = (this._modelHomeDir as string) + texturePath

        // ロード完了時に呼び出すコールバック関数
        const onLoad = (textureInfo: TextureInfo): void => {
          this.getRenderer().bindTexture(modelTextureNumber, textureInfo?.id as WebGLTexture)
  ...
  public update(): void {
    ...
    if (this._motionManager.isFinished()) {
      ...
      if (this._isRandomMotion) {
        this.startRandomMotion(
          LAppDefine.MotionGroupIdle,
          this._isRandomMotion ? LAppDefine.PriorityIdle : LAppDefine.PriorityNone
        )
      } else {
        this.startMotion(LAppDefine.MotionGroupIdle, 0, LAppDefine.PriorityIdle)
      }
    } else {
    ...
    // リップシンクの設定
    if (this._lipsync) {
      let value = this._lipValue // リアルタイムでリップシンクを行う場合、システムから音量を取得して、0~1の範囲で値を入力します。
      if (!this._lipValue) {
  ...
  public startMotion(
    ...
    if (motion == null) {
      fetch(`${this._modelHomeDir}${motionFileName}`)
        ...
        .then((arrayBuffer) => {
          ...
          let fadeTime: number = this._modelSetting?.getMotionFadeInTimeValue(group, no) as number

          if (fadeTime >= 0.0) {
            motion.setFadeInTime(fadeTime)
          }

          fadeTime = this._modelSetting?.getMotionFadeOutTimeValue(group, no) as number
          if (fadeTime >= 0.0) {
      ...
      motion.setFinishedMotionHandler(onFinishedMotionHandler as FinishedMotionCallback)
    }
    ...
    if (voice?.localeCompare('') != 0) {
      let path = voice
      path = (this._modelHomeDir as string) + path
  ...
  public startRandomMotion(
    ...
    const no: number = Math.floor(Math.random() * (this._modelSetting?.getMotionCount(group) || 0))
  ...
  public setExpression(expressionId: string): void {
    const motion: ACubismMotion | null = this._expressions.getValue(expressionId)
  ...
  public hitTest(hitArenaName: string, x: number, y: number): boolean {
    ...
    const count: number = this._modelSetting?.getHitAreasCount() as number
      ...
      for (let i = 0; i < this._modelSetting?.getMotionCount(group); i++) {
        ...
        fetch(`${this._modelHomeDir}${motionFileName}`)
          ...
          .then((arrayBuffer) => {
            ...
            let fadeTime = this._modelSetting?.getMotionFadeInTimeValue(group, i) as number
            if (fadeTime >= 0.0) {
              tmpMotion.setFadeInTime(fadeTime)
            }

            fadeTime = this._modelSetting?.getMotionFadeOutTimeValue(group, i) as number
            if (fadeTime >= 0.0) {
   ...
   * setRandom Motion
   */
  public setRadomMotion(isRandom: boolean): void {
    this._isRandomMotion = isRandom
  }

  /**
   * setLipMotion
   */
  public setLipMotion(number: number): void {
    this._lipValue = number
  }
  ...
  public doDraw(): void {
    ...
    // キャンバスサイズを渡す
    const viewport: number[] = [0, 0, canvas?.width || 0, canvas?.height || 0]
  ...
  _expressions: csmMap<string, ACubismMotion | null> // 読み込まれている表情のリスト
  ...
  _lipValue!: number //set value render lip

  _isRandomMotion: boolean //is set motion random
}
